...
char *send_file(int socket, char *fullfilename)
{
    ...
    puts(fullfilename);
    FILE *picture = fopen(fullfilename, "rb");
...
{
    const char *request_parts2[] = {"select count(ms_id) from message where ch_id = ", ch_id, ";", NULL};
...
char *handle_functions(MYSQL *connection, data_from_json_t data, int sock)
{
    ...
    case 19:
        ...
        break;
    case 20:
        // data.data_array = ["u_id", "new_name", "login"];
        answer = edit_user_name_login(connection, data.data_array[0], data.data_array[1],data.data_array[2]);
        break;
    case 21:
...
void *connection_handler(void *socket_descriptor)
{
    ...
    while ((read_size = recv(socket, client_message, 2000, 0)) > 0)
    {
        ...
        if (strlen(client_message) > 2)
        {
            ...
            if(query.function_number != 31 && query.function_number != 30 && query.function_number != 35 && query.function_number != 8)
...
int main(int argc, char *argv[])
{
    ...
    // get_all_messages(handler_con,"5","0",1);
    // recieve_file()
    ...
    // }
    // edit_user_name_login(handler_con,"15","New_Name","new_login");
    // user_t *user = get_user_info(handler_con,"15");
