...
char *add_message(MYSQL *connection, char *ch_id, char *u_id, char *text, char *is_media, char *isforwarded, char *isreply, char *isseen, char *isedited)
{
    ...
    const char *strings_to_request[] = {"insert into message (ch_id, u_id, ms_text, ms_datetime, ms_ismedia, ms_isforwarded, ms_isreply, ms_isseen, ms_isedited, ms_media) values (", ch_id, ",", u_id, ",\"", text, "\",\"", datetime, "\",", is_media, ",", isforwarded, ",", isreply, ",", isseen, ",", isedited,",\"no_file\" );", NULL};
...
char *get_all_messages(MYSQL *connection, char *ch_id, char *last_msg_id, int socket)
{
    ...
    {
        const char *request_parts2[] = {"select min(ms_id) from message where ch_id = ", ch_id, ";", NULL};
    ...
    if (num_rows > 0)
    {
        char **messages = (char **)malloc(sizeof(char *) * 51);
        int i = 0;
        while (i < num_rows)
        {
            message_t message;
