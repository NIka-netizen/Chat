...
static void activate (GtkApplication *app, gpointer user_data)
{
  ...
  // label and edits
  label_edit_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL,0);
  gtk_container_add (GTK_CONTAINER (window), label_edit_box);
  ...
  for (i = 0; i < 2; i++) {
    ...
    gtk_box_pack_start (GTK_BOX (label_edit_box), label[i], TRUE, 0, 5);
	  gtk_box_pack_start (GTK_BOX (label_edit_box), edit[i], TRUE, 0, 5);
  ...
  gtk_container_add (GTK_CONTAINER (button_box), forg_password);
...
GtkStyleContext *context;
 ...
 gtk_style_context_add_class(context2,"window");
  ...
  //gtk_widget_show (GTK_WIDGET (label_edit_box));
}
