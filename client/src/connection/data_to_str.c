...
char *request_to_server(char *json_str)
{
    ...
    {
        char answer[8192];
        size_t bytes_readed = read(data.socket_desc, answer, 8192);
...
{
    char *data[] = {u_id, name,login, NULL};
...
contact_t **get_all_contacts(char *u_id) // n17
{
    // json_str = {"function_number":"17","data":["u_id"]};
    char *data1[] = {u_id,NULL};
    char *json_str = write_to_json("17", data1);
...
message_arr *get_50_messages(char *ch_id, char *last_msg_id)
{
    ...
    char *data2[] = {ch_id, last_msg_id, NULL};
    char *json_str = write_to_json("8", data2);
    ...
    if (connect(socket_d, (struct sockaddr *)&client_addr, sizeof(client_addr)) == 0)
    {
        //printf("Connected to server, port is %d\n", port);
    }
    else
    ...
    char *response = request_to_get_messages(json_str, socket_d);
    puts(response);
    close(socket_d);
    json_object *jobj, *values_obj, *tmp_values, *values_name;
    jobj = json_tokener_parse(response);
...
message_arr *get_n_messages(char *ch_id, char *n)
{
    ...
    // json_str = {"function_number":"8","data":["ch_id", "last_message_id"]};
    char *data2[] = {ch_id, n, NULL};
    char *json_str = write_to_json("35", data2);
    // response = {"messages": [{ "ch_id": "4", "ms_id": "438", "u_id": "8", "ms_text":
    // "abobus", "ms_datetime": "2022-02-03 13:33:03", "ms_isedited": "0", "ms_isforwarded": "0", "ms_ismedia": "0", "ms_isreply": "0", "ms_isseen": "0" }, ... ]}
